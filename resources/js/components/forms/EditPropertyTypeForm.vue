<template>
    ...
    export default {
       ...
       methods: {
        getPropertyType(){
        axios.get('/api/propertytype/'+this.$route.params.id, {
        }).then((response) => {
            this.form = response.data.propertytype
            // console.log(response)
            console.log("data", this.form)
        })
        },
        submit(){
            axios.put('/api/propertytype/'+this.$route.params.id, this.form)
            .then((response) => {

                    toast.fire({
                    title: 'Property type Updated',
                    text: "property type updated ",
                    icon: 'success',
                })
                this.getPropertyType();
            })
            this.$router.push('/all-propertytypes')
        }
    },
    mounted(){
        this.getPropertyType();
