<template>
    <TheMaster>
        <div class="card px-2">
        <div class="card-body">
        <h5 class="card-title">Edit Condition</h5>
...
export default {
   ...
   data () {
       ...
       methods: {
        getCondition(){
        axios.get('/api/condition/'+this.$route.params.id, {
        }).then((response) => {
            this.form = response.data.condition
            // console.log(response)
            console.log("data", this.form)
        })
        },
        submit(){
            axios.put('/api/condition/'+this.$route.params.id, this.form)
            .then((response) => {
            ...
            this.$router.push('/all-conditions')
        }
    },
    mounted(){
        this.getCondition();
